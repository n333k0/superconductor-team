...
5. Check every slide for `evidence_confidence`, `client_safe`, `needs_input_reason` when required, and `readiness_gate` consistency.
6. Identify blocking issues, material risks, minor corrections, and polish opportunities.
7. Assign all readiness gates: `demo_readiness`, `internal_draft_readiness`, `client_readiness`, and `final_decision`.
8. Quote the validation command or method used for `slides.json` and record the actual result.
9. Spot-check at least five high-impact claims across the synthesis, deck outline, and slides JSON for evidence support.
...
- Any slide with `status: needs-input` or `status: draft` lacks `needs_input_reason`.
- Any slide depends on missing client proof but is not marked `client_safe: false`.
- Any slide is unsafe to use but does not have `readiness_gate: blocked`.
...
- Confirm slide-level metadata follows the rules: internal-note-only slides use `internal-only`; vendor self-positioning is `external-sourced` with caveats; missing client proof is `needs-input`, `client_safe: false`, and blocked when unsafe.
