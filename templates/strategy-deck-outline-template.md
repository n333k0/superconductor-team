# Strategy Deck Outline

Client slug:
Date:
Deck architect:
Intended audience:
Decision the deck must unlock:

## Story Spine

1. Situation:
2. Tension:
3. Evidence:
4. Strategic choice:
5. Offer system:
6. Activation path:

## Section Flow

| Section | Slide Range | Job |
| --- | --- | --- |
|  |  |  |

## Slide Outline

| Slide ID | Section | Assertion Title | Slide Job | Decision It Supports | Key Evidence | Visual Approach | Status | Evidence Confidence | Client Safe | Readiness Gate |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| S01 |  |  |  |  |  |  | draft | internal-only | false | internal-draft-ready |

## Slide Detail

### S01: `<Assertion Title>`

Purpose:

Key message:

Content blocks:

- 

Evidence refs:

- 

Evidence confidence:

Client safe:

Needs input reason:

Readiness gate:

Recommended visual:

Speaker intent:

Production notes:

Missing inputs:

- 

Verifier risk if unresolved:

## Open Production Questions

| Slide ID | Question | Owner | Impact |
| --- | --- | --- | --- |
|  |  |  |  |

## Outline Quality Check

- Slide titles communicate the argument without body copy:
- Every slide has one distinct job:
- Every slide has evidence refs:
- No slide introduces strategy absent from synthesis:
- `needs-input` slides identify the missing input and client-readiness impact:
- Every slide has evidence confidence, client safety, and readiness gate metadata:
- `draft` and `needs-input` slides include `needs_input_reason`:
