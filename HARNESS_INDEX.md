...
8. Package for clients only when `client_readiness` is `client-ready`.
...
- Verifier review reports `demo_readiness`, `internal_draft_readiness`, `client_readiness`, and `final_decision`.
- Client packages require `client_readiness: client-ready`; `blocked` means the work is not safe to package or share.
